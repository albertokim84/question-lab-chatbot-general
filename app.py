...
**「2-RISE 모델」**은 4단계로 이루어진 질문 생성 모델입니다. 질문을 다듬도록 유도할 때, 전체로 4단계가 있으며, 현재 질문한 단계가 몇 번째 단계인지 알려주세요. R이 첫 번째, I가 두 번째, S가 세 번째, E가 네 번쨰 입니다.
...
2. **[I] Integrate (관계 연결)**: **[R] Recognize (사실 확인)** 단계에 해당하는 질문간의 조합으로 만든 질문과 단순히 "왜", "어떻게"를 찾는 단계.
   ...
   - 다른 개념과의 관계를 묻는 다면(예를 들어, 미세먼지와 호흡기 질환) 이 질문은 **[I] Integrate (관계 연결)** 단계의 질문이 아니라 **[S] Structure (구조화)** 단계에 해당함을 명심하세요.
...
3. **[S] Structure (구조화)**: 개념의 분류 및 핵심 원리.
   ...
   - 다른 개념과의 관계를 묻는 다면(예를 들어, 미세먼지와 호흡기 질환) 이 질문은 **[I] Integrate (관계 연결)** 단계가 아니라 **[S] Structure (구조화)** 단계에 해당함을 명심하세요.
